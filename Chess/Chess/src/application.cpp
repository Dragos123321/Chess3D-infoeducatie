...
const double MillisecondsPerSecond = 1'000.;
const double NanosecondsPerSecond = 1'000'000'000.;
...
void Application::start()
{
    ...
    window = glfwCreateWindow(window_width, window_height, "Chess", NULL, NULL);
	...
	std::cout << "\nPress Key 'H' to open the Help file\n\n";
	...
	long long lastTime = getNanos();
	long long lastTimer = getMillis();
	long long now = 0;

	double nanosecondsPerTick = NanosecondsPerSecond / TargetFPS;
	double delta = 0;

	bool shouldRender = false;
	int tps = static_cast<int>(TargetFPS), fps = static_cast<int>(TargetFPS);
...
void Application::renderShadow(int light_index)
{
    ...
    // The model-view-projection matrix is ​​calculated from the point of view of light
    ...
    for (unsigned int i = 0; i < scene.size(); ++i)
    {
        ...
        // Send the matrix to the linked shader (MVP_matrix)
...
void Application::processSelection(int xx, int yy) 
{
    ...
    x_scale = static_cast<float>(framebuffer_width) / window_width;
    y_scale = static_cast<float>(framebuffer_height) / window_height;
    
    glGetIntegerv(GL_VIEWPORT, viewport);
    glReadPixels(static_cast<GLint>(xx * x_scale), viewport[3] - static_cast<GLint>(yy * y_scale), 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &res);
...
void Application::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (action == GLFW_PRESS) {
        ...
        case 'E':
            ...
            break;
		case 'H':
			system("start Help_files/Help.chm");
			break;
        case 'T':
            saveTexture();
            break;
...
void Application::mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) 
	{
        ...
        processSelection(static_cast<int>(x), static_cast<int>(y));
    }
}

void Application::setTitleFps(int fps)
{
    std::string title = "Chess - FPS: " + std::to_string(fps) + " - Player " + std::to_string(game.getPlayerId());
