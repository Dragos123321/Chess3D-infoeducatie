...
std::vector<std::vector<Piece*>> Board::initWithFile(Scene* scene, std::string filename) 
{    
    m_scene = scene;
    ...
    for (int i = 0; i < 2; ++i)
	{
        for (int j = 0; j < static_cast<int>(pieces[i].size()); ++j)
		{
            ...
            m_vaoIDsMap[vaoIDs[indice]] = pieces[i][j];
        }
    }
...
std::vector<std::vector<Piece *> > Board::initClassic(Scene* scene) {
    ...
    std::vector<int> vaoIDs = m_scene->addVaoPieces(model,team,pos);
...
{
    return m_centerToSquare0 + glm::vec3(j * m_squareOffset, 0.0f, i * m_squareOffset);
}

void Board::computeAllSquares()
{
    m_squares = std::vector<std::vector<glm::vec3>>(8);

    for(int i = 0; i < 8; ++i)
	{
        m_squares[i].resize(8);
        for(int j = 0; j < 8; ++j)
            m_squares[i][j] = computeRealPosition(i,j);
    }

}
void Board::movePieceTo(int vao, int i, int j)
{
    Piece* piece = m_vaoIDsMap[vao];

    if (piece == nullptr) throw std::string("The piece requested does not exist");

    if(piece->getName() == "Knight")
	{
        m_scene->jumpVAOTo(vao - 1, getPosAt(i, j));
    }
	
	else
	{
        m_scene->slideVAOTo(vao - 1, getPosAt(i, j));
    }
